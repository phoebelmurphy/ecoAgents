...
public class Grid extends GridWorldModel {
	...
	Logger logger;
	public Grid(int x, int y, Logger logger){
		super(x,y, 3);
		this.x = x;
		this.y = y;
		this.logger = logger;
	...
	public boolean moveAgent(Agent agent, int newx, int newy){
		...
		if ((newx < 0) || (newy < 0) || (newx >= x) || (newy >= y)) {
			...
			logger.logp(Level.SEVERE, "Grid", "moveAgent",
					message);
			result = false;
		} else {
			result = true;
			...
			logger.logp(Level.INFO, "Grid", "moveAgent",
					"moving to position "
	...
	/**
	 * Converts a char direction indicator to coordinates,
	 * for example 'l' would be converted to (-1, 0).
